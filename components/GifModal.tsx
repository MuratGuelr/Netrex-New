...
interface GifModalProps {
  currentUser: User;
  onClose: () => void;
  onSend: (gifUrl: string, gifTitle: string) => void;
}
...
export default function GifModal({ currentUser, onClose, onSend }: GifModalProps) {
  const [activeTab, setActiveTab] = useState<'trending' | 'favorites'>('trending');
  const [gifs, setGifs] = useState<Gif[]>([]);
  const [favoriteGifs, setFavoriteGifs] = useState<Array<{ id: string; url: string; title: string }>>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(favoritesRef, (snapshot) => {
      const favorites: Array<{ id: string; url: string; title: string }> = [];
      snapshot.forEach((doc) => {
        favorites.push({ id: doc.id, ...doc.data() } as { id: string; url: string; title: string });
      });
